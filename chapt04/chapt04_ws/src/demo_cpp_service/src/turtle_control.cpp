...
{
private:
    rclcpp::Service<Patrol>::SharedPtr patrol_service_;  //巡逻服务的智能指针
        ...
        patrol_service_ = this->create_service<Patrol>(
            ...
            std::shared_ptr<Patrol::Response> response) -> void 
            {
                if(
                    ...
                    response->result = Patrol::Response::SUCESS;  //设置响应结果为成功
                }
                else
                {
                    RCLCPP_ERROR(this->get_logger(), "Invalid patrol request: target_x=%.2f, target_y=%.2f", request->target_x, request->target_y);
                    response->result = Patrol::Response::FAIL;  //设置响应结果为失败
                }
